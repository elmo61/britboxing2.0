...
// Completed fights have their own page (/results) — this page is upcoming
// fights only.
const upcoming = computed(() => (bouts.value ?? []).filter((b: any) => b.status !== 'completed'))
const hasResults = computed(() => (bouts.value ?? []).some((b: any) => b.status === 'completed'))
...
<template>
  <div class="wrap wrap--wide">
    ...
    <p v-if="hasResults" class="sched__results-link">
      <NuxtLink to="/results">See how recent fights turned out →</NuxtLink>
    </p>
  </div>
</template>
...
.sched__results-link {
  font-family: var(--font-cond); font-weight: 600; letter-spacing: .06em;
  text-transform: uppercase; font-size: .82rem; margin-top: 26px;
}
.sched__results-link a { color: var(--gold); text-decoration: none; }
